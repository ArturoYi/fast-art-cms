...
const list = ref<BlogTag[]>([]);

const { run: fetchList, loading: listLoading } = useRequest(getBlogTagListService, {
  manual: true,
  onSuccess: res => {
    list.value = res?.data ?? [];
  },
...
const columns = computed<DataTableColumns<BlogTag>>(() => [
...
const { run: createOne, loading: saving } = useRequest(createBlogTagService, {
  manual: true,
  onSuccess: () => {
...
const { run: updateOne, loading: updating } = useRequest(updateBlogTagService, {
  manual: true,
  onSuccess: () => {
...
const { run: deleteOne, loading: deleting } = useRequest(deleteBlogTagService, {
  manual: true,
  onSuccess: () => {
...
function confirmDelete(row: BlogTag) {
  dialog.warning({
    title: '确认删除？',
    content: `将删除标签「${row.name}」`,
    positiveText: '删除',
    negativeText: '取消',
    onPositiveClick: () => {
      deleteOne(row.id);
