...
function initCacheFromStorage() {
  try {
    ...
    console.warn("[Cache] Failed to parse manifest", e);
    manifestSet = new Set();
  }
  ...
  manifestSet.forEach((key) => {
    ...
    try {
      ...
      console.warn(`[Cache] Failed to restore key: ${key}`, e);
      keysToRemove.push(key);
    }
  });
...
function saveManifest() {
  try {
    localStorage.setItem(
      CACHE_MANIFEST_KEY,
      JSON.stringify(Array.from(manifestSet))
    );
  } catch (e) {
    console.warn("[Cache] Failed to save manifest", e);
  }
}
 ...
 */
export function getCache<TData>(
  key: string,
  cacheTime: number
): TData | undefined {
  // 直接从内存读取（初始化时已同步）
