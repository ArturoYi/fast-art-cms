...
import type { Service, UseRequestOptions, UseRequestResult } from "./types";
import { FetchClientError } from "../utils/types";
import {
  debounce,
  getCache,
  getCacheTimestamp,
...
export function useRequest<TData, TParams extends any[] = []>(
  ...
  /** 当前最近一次请求使用的参数 */
  const params = ref<TParams | undefined>(defaultParams) as Ref<
    TParams | undefined
  >;
  ...
  const fetchData = async (
    args: TParams,
    retryRemaining: number = retryCount
  ): Promise<TData> => {
    ...
    try {
      ...
      const fetchError =
        err instanceof FetchClientError
          ? err
          : new FetchClientError("OTHER_ERROR", String(err));
